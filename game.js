...
var fullScreenText = new PIXI.Text("", {
    font: "60px 'Bree Serif'",
    fill: "#FFFFFF",
    align: "center"
});
...
function storyTextReset() {
    ...
    storyModeTextBlock.style = {
        fill: "#FFFFFF",
        font: "20px 'Bree Serif'",
        align: "left"
    };
    storyModeTextBlock.position.set(50, 26);
    storyModeTextBlock.anchor = {
        x: 0,
        y: 0
    };
}
...
var storyModeText = new Howl({ // Played once on "Story"
    ...
    volume: 1,
    onend: function() {
        storyModeTextBlock.position.set(width / 2, height / 2);
        storyModeTextBlock.anchor = {
            x: 0.5,
            y: 0.5
        };
        storyModeTextBlock.style.align = "center";
        storyModeTextBlock.text = " --- YOU ARE THE MONSTER ---\nFind your loved one!\nBy hugging everyone as hard as you can!\n --- BUT BEWARE OF THE POLICE ---";
...
function setup() {
    ...
    var menuButtons = [
        [new PIXI.Sprite.fromImage("Images/buttons/story.png"), function(data) {
            changeState(story)
        }],
        [new PIXI.Sprite.fromImage("Images/buttons/easy.png"), function(data) {
            hardness = 0;
            changeState(play);
        }],
        [new PIXI.Sprite.fromImage("Images/buttons/normal.png"), function(data) {
            hardness = 1;
            changeState(play)
        }],
        [new PIXI.Sprite.fromImage("Images/buttons/hard.png"), function(data) {
            hardness = 2;
            changeState(play)
        }],
        [new PIXI.Sprite.fromImage("Images/buttons/endless.png"), function(data) {
            hardness = 3;
            changeState(play)
        }],
    ]
    ...
    creditButton.click = function(data) {
        changeState(credit)
    };
...
function Police() {
    ...
    this.setShoot = function(bool) {
        if (bool == this.shooting) {
            return undefined
        };
        this.shooting = bool;
        this.shoot();
    ...
    this.init = function() {
        ...
        this.sprites.shoot.pivot = {
            x: 0.5,
            y: 0.5
        };
        this.sprites.shoot.anchor = {
            x: 0.5,
            y: 0.5
        };
        this.sprites.shoot.scale = {
            x: 0.5,
            y: 0.5
        };
        this.sprites.walk.pivot = {
            x: 0.5,
            y: 0.5
        };
        this.sprites.walk.amchor = {
            x: 0.5,
            y: 0.5
        };
        this.sprites.walk.scale = {
            x: 0.5,
            y: 0.5
        };
...
function Casult() {
    ...
    for (var i = this.sprites.length - 1; i >= 0; i--) {
        this.sprites[i].scale = {
            x: 0.5,
            y: 0.5
        };
        this.sprites[i].anchor = {
            x: 0.5,
            y: 0.5
        };
        this.sprites[i].pivot = {
            x: 0.5,
            y: 0.5
        };
    };
